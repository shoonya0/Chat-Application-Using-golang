	...
	return (
		<>
			<Dialog fullWidth maxWidth="md" open={open} onClose={handleClose} keepMounted TransitionComponent={Transition} sx={{ p: 4 }}>
							...
							<Grid item xs={6} key={key} container>
								<Stack sx={{ width: "100%" }} justifyContent="space-between" spacing={3} direction="row" alignItems="center">
									...
									<Stack spacing={2} direction={"row"}>
										{combination.map((ele) => { return <Button disabled variant="contained" sx={{ color: "#212121" }}>{ele}</Button> })}
									</Stack>
								</Stack>
							</Grid>
						)}
				...
				{/* here comes the dialog action in this case ok button to go the the previous state */}
				<DialogActions>
